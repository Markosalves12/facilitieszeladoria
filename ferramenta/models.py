...
def generate_id_random():
    while True:
        ...
        return random_int
class CatalogoFerramentas(models.Model):
    ...
    status = models.CharField(max_length=60, blank=False, null=False, choices=status_options, default='Mobilizado')
    ...
    empresa = models.ForeignKey(
        ...
        null=False,
        blank=False,
        related_name='empresacatalogoferramenta'
    )
    # esses 3 campos juntos funcionam como chave primaria
    # evitando duplicatas desses campos combinados
    class Meta:
...
class FerramentaDisponivel(models.Model):
    id_random = models.IntegerField(
        unique=True,
        default=generate_id_random
    )
    ...
    matricula = models.CharField(max_length=100, blank=False, null=False, unique=True)

    catalogo_ferramenta = models.ForeignKey(
        to=CatalogoFerramentas,
        on_delete=models.CASCADE,
        blank=False,
        null=False,
        related_name="catalogoferramentaoriginal"
    )
    ...
    empresa = models.ForeignKey(
        ...
        blank=False,
        null=False,
        related_name='empresaferramentaorigem'
    )
    ...
    tipo = models.CharField(max_length=60, blank=False, null=False, choices=tipo_options, default='Proprio')

    def __str__(self):
        return f"{self.catalogo_ferramenta} | {self.matricula} | {self.empresa} | {self.tipo}"


class ManutencaoFerramentas(models.Model):
    id_random = models.IntegerField(
        unique=True,
        default=generate_id_random
    )
    ...
    tipo_manutencao = models.CharField(max_length=60, blank=False, null=False, choices=opcoes_manutencoes, default='')
